import { pgTable, serial, text, real, timestamp, integer, boolean, primaryKey, jsonb } from 'drizzle-orm/pg-core'
...
// ==============================================
// HILERAS — líneas de plantación con mapa
// ==============================================
export const hileras = pgTable('hileras', {
  id: serial('id').primaryKey(),
  variedadId: integer('variedad_id').references(() => variedades.id),
  variedadAId: integer('variedad_a_id').references(() => variedades.id),
  variedadBId: integer('variedad_b_id').references(() => variedades.id),
  split: boolean('split').default(false).notNull(),
  poste: text('poste'),
  plantas: integer('plantas'),
  lat1: real('lat1').notNull(),
  lng1: real('lng1').notNull(),
  lat2: real('lat2').notNull(),
  lng2: real('lng2').notNull(),
  longitudM: real('longitud_m').notNull(),
  anchoM: real('ancho_m').default(2.5).notNull(),
  anio: integer('anio').default(2024).notNull(),
  notas: text('notas'),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
})

export const hilerasRelations = relations(hileras, ({ one }) => ({
  variedad: one(variedades, {
    fields: [hileras.variedadId],
    references: [variedades.id],
  }),
  variedadA: one(variedades, {
    fields: [hileras.variedadAId],
    references: [variedades.id],
  }),
  variedadB: one(variedades, {
    fields: [hileras.variedadBId],
    references: [variedades.id],
  }),
}))
...
// ==============================================
// ADMINS — emails autorizados para acceder
// ==============================================
